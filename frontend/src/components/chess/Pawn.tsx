...
export const Pawn = ({ color, position, onClick }: { color: ChessColor, position: ChessPosition, onClick: (pos: ChessPosition) => void }) => {
  const pawn = new ChessPiece("pawn", color, position, 50, 600, 75);

  return (
    <svg x={pawn.boardPosition.x} y={pawn.boardPosition.y}
      width={pawn.tileSize} height={pawn.tileSize} style={{ overflow: 'visible' }} onClick={() => onClick(position)}>
