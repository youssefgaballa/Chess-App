...
export const UserForm = ({handleSubmit, errorMessage, setErrorMessage, type }: {
  handleSubmit: (event: React.FormEvent<HTMLFormElement>, userData: UserData) => Promise<void>,
  errorMessage: string,
  ...
  return (
        ...
        <form onSubmit={(event) => handleSubmit(event,{username, email, firstName, lastName, password, role})} className='flex flex-col items-center justify-start text-center mt-[3%] mb-[3%] w-full h-[85%]'>
          <span className='font-bold text-5xl mb-[3%]'>{type === 'register' ? 'User Registration' : 'Edit User Details'}</span><br />
          <span className='text-xl'>{type === 'register' ? 'Sign up for a free user account.' : 'Edit your user account.'}</span><br />
          ...
          <button type='submit' disabled={!isUsernameValid || !isPasswordValid || !isConfirmPasswordValid ? true : false}
            className={`mt-[2%] mb-[2%] w-[25%] p-[1.5%] bg-blue-500 text-white rounded-md
              ${!isUsernameValid || !isPasswordValid || !isConfirmPasswordValid ? 'cursor-not-allowed bg-gray-400' : 'hover:bg-blue-700 cursor-pointer'}`}>{type === 'register' ? 'Register' : 'Save Changes'}</button>
          <br />
          <div hidden={errorMessage ? false : true}><span hidden={errorMessage ? false : true} ><Error style={{ fill: 'red' }} /></span>{errorMessage}</div>
         {type === 'register' && <div>Already have an account? <Link to="/Login" state={{ from: location }} className='text-blue-500'>Login</Link></div>}
        </form>
